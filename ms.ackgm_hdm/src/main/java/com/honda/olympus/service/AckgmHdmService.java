...
public class AckgmHdmService {
	...
	public void callAckgmHd() {
		...
		for(MaxTransitResponseVO response:maxTransit)
		{
			...
			Long rqstIdentifier = Long.parseLong(response.getRqstIdentifier());
			...
			try {
				...
				event = new EventVO(serviceName, AckgmConstants.ZERO_STATUS, "Fallo de inserción en la tabla AFE_ORDER_HISOTRY", "");
				logEventService.sendLogEvent(event);
